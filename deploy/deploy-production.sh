...
WORKWEAR_IMAGE="${WORKWEAR_IMAGE:-$(read_env_value WORKWEAR_IMAGE)}"
export WORKWEAR_IMAGE
: "${WORKWEAR_IMAGE:?WORKWEAR_IMAGE is required}"
immutable_image_pattern='^ghcr\.io/dimonichise/[a-z0-9._/-]+:sha-[0-9a-f]{40}$'
[[ "$WORKWEAR_IMAGE" =~ $immutable_image_pattern ]] || { echo "WORKWEAR_IMAGE must use an immutable sha-<40 hex> GHCR tag." >&2; exit 1; }
...
verify() {
  ...
  [[ "$(docker inspect --format '{{.Config.Image}}' "$("${compose[@]}" ps -q app)")" == "$WORKWEAR_IMAGE" ]]
...
rollback() {
  ...
  docker image tag "$old_image_id" "$WORKWEAR_IMAGE" >/dev/null 2>&1 || true
  "${compose[@]}" up -d --no-build --no-deps app >/dev/null 2>&1 || true
}
trap rollback EXIT

docker image tag "$old_image_id" ghcr.io/dimonichise/workwear-turnover:rollback
docker pull "$WORKWEAR_IMAGE"
...
printf 'revision=%s\ndeployed_at=%s\nbackup=created\nimage=%s\n' "${WORKWEAR_IMAGE##*:sha-}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$WORKWEAR_IMAGE" > "$state_dir/last-successful"
completed=1
trap - EXIT
